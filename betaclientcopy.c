...
int main(int argc, char *argv[])
{
	...
	*Server;	/* server host */
    ...
    int action,started,bot2,bot3,bot4;
    action = 0;
    started = 0;
    bot2 = 0;
    bot3 = 0;
	...
	printf("action is %d\n", action);
	printf("Bot status: %d", players[action].bot);

	if(players[action%3].bot == 1)
	{
		sprintf(SendBuf, "CALL %d", action);
			
	} 	
	
	else if(players[action%3].bot == 0)
	{
	...
	if(strncmp(SendBuf, "START", 5) == 0)
        {
                started = 1;
                sscanf(RecvBuf + 10, "%1d%1d%1d", &bot2, &bot3, &bot4);
		printf("bot2= %d\n", bot2);
		printf("bot3= %d\n", bot3);
		printf("bot4= %d\n", bot4);
                players[0].bot = 0;
