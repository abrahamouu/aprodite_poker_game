	...
	int DataSocketFD)
{
    ...
    /*player chooses seat*/
    if(0 == strncmp(RecvBuf, "GETSEAT ", 8))
    {
        
        /*getSeat Function*/
        strncpy(player.position, RecvBuf + 8, sizeof(player.position) - 1);
        strncpy(SendBuf, "SEAT CHOSEN! ", sizeof(SendBuf)-1);
        SendBuf[sizeof(SendBuf)-1] = 0;
    }

     /*player sets username*/
    else if(0 == strncmp(RecvBuf, "ENTER ", 6))
    {
    
        strncpy(player.username, RecvBuf + 6, sizeof(player.username) - 1);
        strncpy(SendBuf, "USERNAME SET! ", sizeof(SendBuf)-1);
        SendBuf[sizeof(SendBuf)-1] = 0;
    }
