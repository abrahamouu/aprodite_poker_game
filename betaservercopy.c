...
void ProcessRequest(int DataSocketFD, TABLE *table) {
    ...
    else if(game.currRound <= READY && (strncmp(RecvBuf, "START", 5) == 0))
    {
	...
	if(game.totalPlayers < 4)
	{
		...
		{
			players[4-(game.totalPlayers)].bot = 1;	
			if(counter == 0)
			{
				a = 4-(game.totalPlayers);
			}
			else if(counter == 1)
			{
				b = 4-(game.totalPlayers);
			}
			else if(counter == 2)
			{
				c = 4-(game.totalPlayers);
			}		
			game.totalPlayers --;
		...
		snprintf(SendBuf, sizeof(SendBuf), "Bot index:%d%d%d. Rest of the players are replaced with poker bots. Have fun!\n", a,b,c);
	    ...
	    snprintf(SendBuf, sizeof(SendBuf), "Current pot is %d. Player %s, You are Big blind. Please initialize your bet", table->pot, players[1].username);
	...
	else if(strncmp(RecvBuf, "BET 2 ", 6) == 0)
	{
		...
		players[1].bet = userBet;
           	...
           	players[1].points = players[1].points - userBet;
                ...
                game.actioncounter += 1;
		snprintf(SendBuf, sizeof(SendBuf), "Current pot is %d. Player %s, make your next action", table->pot, players[2].username);
	}
	...
	if(players[seat].points < (raiseAmt + table->minBet))
	{
        ...
        switch(seat){
			...
			table->prevminBet = table->minBet;
                        ...
                        snprintf(SendBuf, sizeof(SendBuf), "Player %s has raised by %d.", players[0].username, raiseAmt);
			...
			table->prevminBet = table->minBet;
                        ...
                        snprintf(SendBuf, sizeof(SendBuf), "Player %s has raised by %d.", players[1].username, raiseAmt);
                        printf("Player 2 raised by %d.\n", raiseAmt);
			...
			table->prevminBet = table->minBet;
                        ...
                        snprintf(SendBuf, sizeof(SendBuf), "Player %s has raised by %d.", players[2].username, raiseAmt);
			...
			table->prevminBet = table->minBet;
                        ...
                        snprintf(SendBuf, sizeof(SendBuf), "Player %s has raised by %d.", players[3].username, raiseAmt);
                        printf("Player 3 raised by %d.\n", raiseAmt);
